...
const booksEl = document.getElementById('card');

function createBookDiv(bookObject) {
  let divEl = document.createElement('div');
  divEl.className = 'box col-3 text-center border border-info rounded';
  divEl.innerHTML = `<img src="img/${bookObject.coverFileName}" class="images rounded-top" alt="${bookObject.title}">
    ...
    <p class="book__price">${bookObject.price}</p>
    <button type="button" class="book__btn btn btn-outline-info" data-id="${bookObject.bookId}">Add to cart</button>`;
  return divEl;
};
...
const catBtnsEl = document.getElementsByClassName('category__btns')[0];
if (catBtnsEl) {
  catBtnsEl.addEventListener('click', function (event) {
    ...
    // if (!event.target.className.includes('activ')) {
...
function searchBook(searchText) {
  let resultArr = books.filter(el => {
    return el.title.toLowerCase().includes(searchText) || el.author.toLowerCase().includes(searchText);
  });
  return resultArr;
};
...
if (booksEl) {
  booksEl.addEventListener('click', function (event) {
    if (event.target.className.includes('book__btn'));
...
function createCartItemTr(el) {
  ...
  trEl.innerHTML = `
  <td> <img src="img/${el.coverFileName}" alt="${el.title}" class="img"> </td>
  <td class="book__title"> ${el.title} </td>
  <td class="book__price"> ${el.price} </td>
  <td class="align-middle"> <button type="button" data-id="${el.bookId}" class="delete-btn btn btn-danger"> Delete </button> </td>`;
  return trEl;
};

if (cartItemsEl) showCartItems();
